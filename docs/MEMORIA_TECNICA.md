...
### 4.9 Módulo de categorías en el admin (Metronic)

Una vez resuelto el módulo de autenticación, el siguiente bloque fue el **CRUD de categorías** en el panel de administración.

La estructura de categorías sigue tres niveles jerárquicos: Departamento (primer nivel), Categoría (segundo nivel) y Subcategoría (tercer nivel). Esto se refleja en la tabla `categories` de la base de datos mediante dos columnas de FK autorreferenciadas: `categorie_second_id` y `categorie_third_id`. El modelo `Categorie` usa `SoftDeletes` para preservar el historial y tiene los timestamps sobreescritos para usar la zona horaria `Europe/Madrid`.

Generé los componentes con `ng generate`:  `CreateCategorieComponent`, `ListCategorieComponent`, `EditCategorieComponent` y `DeleteCategorieComponent`, más el `CategoriesService`. El módulo los declara todos e importa `HttpClientModule`, `FormsModule`, `ReactiveFormsModule`, `NgbModule` y `NgbModalModule`.

Algunos detalles técnicos que me surgieron durante esta fase:

- **Tipo `string | ArrayBuffer | null` incompatible con `[src]`**: el `FileReader.result` está tipado en TypeScript como `string | ArrayBuffer | null`, aunque `readAsDataURL` siempre devuelve `string`. Angular strict templates no acepta `ArrayBuffer` como valor para el atributo `[src]` de una imagen. La solución fue cambiar la propiedad a `string | null` y castear el resultado con `as string`.

- **`==` vs `===` en templates Angular**: el compilador de Angular en modo estricto prefiere `===` en las directivas `[ngClass]`. Cambié las tres comparaciones `type_categorie == N` por `===` para eliminar los errores en el editor.

- **Rutas en `api.php`**: las rutas del CRUD de categorías se añaden en un grupo con `middleware auth:api` y `prefix admin`, usando `Route::resource()`. El controlador ya estaba creado en `App\Http\Controllers\Admin\Product\CategorieController`.

- **`CategoriesService`**: añadí `isLoading$` como `Observable<boolean>` derivado de un `BehaviorSubject`, y los métodos `listCategories(page, search)` y `deleteCategorie(id)` como llamadas HTTP a `/api/admin/categories`. El flag de carga se activa con `finalize()` del pipeline de RxJS.

- **Modal de eliminación**: `DeleteCategorieComponent` se abre desde el listado mediante `NgbModal`. Recibe la categoría como `@Input()` y emite un `@Output() CategorieD` cuando el usuario confirma. El listado se suscribe a ese evento y actualiza la vista localmente (sin necesidad de recargar toda la lista desde la API).

- **Vista de árbol en `ListCategorieComponent`**: la API devuelve las categorías en formato plano y el componente las convierte a jerarquía en el cliente usando un `Map<id, nodo>`. Cada nodo tiene un booleano `expanded` para controlar la apertura/cierre del árbol. Los métodos `expandAll` y `collapseAll` recorren el árbol recursivamente.
...
- **Guards en producción**: los guards del admin están implementados pero podrían reforzarse con lógica de roles más granular.
...
### 8.1 Decisión de arquitectura: por qué Docker

Durante el desarrollo todo corría en local con tres terminales abiertas: `php artisan serve` en el puerto 8000, `ng serve` en el 4200 y otro `ng serve` en el 4201. Eso funciona perfectamente para desarrollar, pero no es una forma razonable de presentar un proyecto. Si el evaluador tiene que instalar XAMPP, PHP 8.2, Composer, Node 18 y dos versiones distintas de Angular solo para ver la app funcionar, estamos poniendo barreras innecesarias antes de llegar a lo que importa.

La solución que adopté fue empaquetar toda la aplicación en **contenedores Docker** orquestados con `docker-compose`. La idea es que arrancar el proyecto entero sea un único comando, independientemente del sistema operativo del evaluador.

Para la presentación, preparé además una **máquina virtual Ubuntu 22.04** en VirtualBox donde la aplicación ya está levantada y configurada. Así la demostración no depende de tener internet ni de que el ordenador del tribunal tenga Docker instalado.

### 8.2 Arquitectura de contenedores

El `docker-compose.yml` define cinco servicios que se comunican entre sí a través de una red interna de Docker:

```
lovestonedgirls/
├── docker-compose.yml          ← orquestación de todos los servicios
├── nginx/
│   └── nginx.conf              ← proxy inverso (enruta dominios)
├── db/
│   └── init.sql                ← volcado de la base de datos
├── api_ecommerce/
...
│   └── nginx.conf              ← configuración del servidor Angular
└── admin_metronic_8.2/
    ├── Dockerfile              ← Node 18 (build) + Nginx (serve)
    └── nginx.conf              ← configuración del servidor Angular
```

**`db`** — imagen oficial `mysql:8.0`. Al arrancar por primera vez importa automáticamente el archivo `db/init.sql` con todos los datos de la base de datos. Las tablas, usuarios y productos quedan listos sin ningún paso adicional.

**`api`** — construido desde `api_ecommerce/Dockerfile`. Usa una imagen `php:8.2-cli`, instala Composer e instala las dependencias con `composer install`. El servidor arranca con `php artisan serve --host=0.0.0.0 --port=8000`. Las variables de entorno de producción (conexión a base de datos, JWT secret, credenciales de Gmail, Stripe) vienen del archivo `.env.docker`.

**`ecommerce`** y **`admin`** — ambos usan un Dockerfile de construcción en dos etapas (*multi-stage build*). La primera etapa usa `node:18-alpine` para ejecutar `npm install` y `npm run build`, compilando los ficheros estáticos de Angular. La segunda etapa copia únicamente el resultado del build a una imagen `nginx:alpine` mucho más ligera. Esto hace que la imagen final no contenga Node ni las `node_modules` (que pesan varios gigabytes), sino solo los ficheros HTML, CSS y JavaScript ya optimizados.

**`proxy`** — un contenedor Nginx que actúa como punto de entrada único en el puerto 80. Lee el dominio de cada petición y la reenvía al contenedor correspondiente:

```nginx
server { server_name lovestonedgirls.daw;       → proxy_pass http://ecommerce:80; }
server { server_name admin.lovestonedgirls.daw; → proxy_pass http://admin:80;     }
server { server_name api.lovestonedgirls.daw;   → proxy_pass http://api:8000;     }
```

### 8.3 Resolución de dominios locales

Los dominios `lovestonedgirls.daw`, `admin.lovestonedgirls.daw` y `api.lovestonedgirls.daw` no existen en el DNS público. Para que el sistema operativo los resuelva hacia `127.0.0.1` (la propia máquina) es necesario añadirlos al archivo `/etc/hosts`:

```
127.0.0.1 lovestonedgirls.daw
127.0.0.1 admin.lovestonedgirls.daw
127.0.0.1 api.lovestonedgirls.daw
```

En la máquina virtual ya están configurados. Si alguien quiere probarlo en otro sistema, este es el único paso manual que hay que hacer.

### 8.4 Variables de entorno de producción

El archivo `.env.docker` contiene la configuración específica para el entorno Docker. Los cambios más importantes respecto al `.env` de desarrollo son:

- `DB_HOST=db` — en vez de `127.0.0.1`. Dentro de la red Docker, los servicios se identifican por su nombre de servicio, no por dirección IP.
- `DB_PASSWORD=root` — la contraseña definida en el `docker-compose.yml` para el contenedor MySQL.
- `APP_URL=http://api.lovestonedgirls.daw` — URL pública de la API.
- `SESSION_DRIVER=file` y `CACHE_STORE=file` — evitan depender de tablas de base de datos para sesiones y caché, simplificando el arranque.

### 8.5 Manual de despliegue: paso a paso

#### Para arrancar la aplicación por primera vez

```bash
# 1. Clonar el repositorio
git clone https://github.com/Isabel96AG/lovestonegirls.git
cd lovestonegirls

# 2. Añadir los dominios al /etc/hosts (solo la primera vez)
sudo nano /etc/hosts
# Añadir al final:
# 127.0.0.1 lovestonedgirls.daw
# 127.0.0.1 admin.lovestonedgirls.daw
...
La primera vez tarda entre 15 y 30 minutos porque tiene que descargar las imágenes base y compilar los dos proyectos Angular. Las siguientes veces arranca en menos de un minuto.

#### Para arrancar sin reconstruir (uso habitual)
...
Una vez arrancado, la aplicación es accesible en:

| URL | Descripción |
|-----|-------------|
| `http://lovestonedgirls.daw` | Tienda pública |
| `http://admin.lovestonedgirls.daw` | Panel de administración |
| `http://api.lovestonedgirls.daw/api` | API REST |

### 8.6 Manual para el evaluador

La forma más sencilla de evaluar el proyecto es a través de la **máquina virtual** que se entrega junto con el código. No requiere tener Docker instalado ni configurar nada.

**Requisitos:** VirtualBox (disponible gratis en virtualbox.org para Windows, Mac y Linux).

**Pasos:**

1. Instalar VirtualBox si no está instalado.
2. Abrir VirtualBox → `Archivo → Importar servicio virtualizado` → seleccionar el archivo `LovestonedGirls.ova`.
3. Dejar todas las opciones por defecto y pulsar `Importar`.
4. Seleccionar la máquina `LovestonedGirls` en la lista y pulsar `Iniciar`.
5. Esperar a que cargue el escritorio de Ubuntu (puede tardar 1-2 minutos).
6. Abrir **Firefox** desde el escritorio de Ubuntu.
7. Navegar a `http://lovestonedgirls.daw`.

La aplicación ya está completamente levantada dentro de la VM: la base de datos tiene datos de ejemplo, los usuarios de prueba existen y los contenedores arrancan automáticamente al iniciar la máquina.

**Credenciales de prueba:**

| Rol | Email | Contraseña |
|-----|-------|-----------|
| Administrador | admin@lovestonedgirls.com | password |
| Cliente | cliente@lovestonedgirls.com | password |

El panel de administración es accesible en `http://admin.lovestonedgirls.daw`.

### 8.7 Guía para la defensa

El día de la presentación el flujo será:

1. **Conectar el disco duro externo** (TOSHIBA EXT) al MacBook antes de arrancar VirtualBox.
2. **Abrir VirtualBox** → seleccionar `LovestonedGirls` → pulsar `Iniciar`.
3. Esperar al escritorio de Ubuntu (1-2 minutos).
4. **Abrir Firefox** dentro de la VM.
5. Navegar a `http://lovestonedgirls.daw` para mostrar la tienda.
6. Navegar a `http://admin.lovestonedgirls.daw` para mostrar el panel.

Puntos que conviene tener preparados para demostrar:

- **Registro de usuario**: crear una cuenta nueva y verificar el email (el correo llega a la bandeja de Gmail del proyecto).
- **Login y sesión**: iniciar sesión como cliente, añadir productos al carrito y completar un pago de prueba con Stripe (usar el número de tarjeta `4242 4242 4242 4242`).
- **Panel de administración**: mostrar el listado de productos, categorías y pedidos. Crear una nueva categoría.
- **Arquitectura**: tener abierta la terminal con `docker-compose logs -f` para mostrar que los cuatro servicios están corriendo.
